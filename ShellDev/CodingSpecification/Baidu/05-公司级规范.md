...
# Good
# 涉及空格，需要加引号。
# 如果不加引号，s1中的world会被当成独立命令并报错，s2中更是会删除名为file的文件，s1和s2都不会被赋值。s
s1="Hello world"s
s2="I rm file"
...
# Bad 
# 涉及shell元字符，未加引号。&会被视为控制字符，其后的url参数全部无效，并且将wget进程挂到后台。w
...
# Bad 
# 路径中存在空格，不加引号会被当成两个路径并引发预期外的错误。l
ls -l /home/work/my space/workspace

# 使用变量拼接了命令选项，并按RULE 6-9加了引号，这会造成命令选项无法被识别。确认作为命令选项时，不要引用。p
param="-l -r -t"l
ls "${param}" /home/work
...
# Bad 
# s3和s4的值分别是$PWD和$(pwd)这两个字符串，除非有特殊需求，否则是错误的。s
s3='$PWD's
s4='$(pwd)'
...
# Good  
# 整数字面量不引用，纯数字字符串需要引用。v
value=32i
id="0123456789012345"

# Bad 
# 使用了命令替换，即使是整形，但仍然应该引用。n
...
# Bad
# 会把prefix_dir视为变量名，由于无定义，会输出/file，若设置了set -u，则会报错。p
prefix="pre"
